...
import Reproductor.Librerias.Direcciones;
...
Cargar_audio audio;
    Busqueda(ListaDobleConOrden ldco) {
        Busqueda.ldco = ldco;
        initComponents();
        setLocation(400, 400);
        setSize(445, 400);
    ...
    public void Reproduce(File Direccion_Archivo) {
        ...
        for (int h = 0; h < filas; h++) {
            if (ruta == null ? Libreria_Tabla.getMiTabla().getValueAt(h, 1).toString() == null : ruta.equals(Libreria_Tabla.getMiTabla().getValueAt(h, 1).toString())) {
                audio.Pista = (h);//resuelto por que no enviaba el numero de pista
                audio.Reproducir(h);
    ...
    void setAudio(Cargar_audio audio) {
        this.audio=audio;
    }
}
