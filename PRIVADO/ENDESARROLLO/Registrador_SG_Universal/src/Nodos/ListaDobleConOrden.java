...
public class ListaDobleConOrden {
    ...
    public void agrega(Direcciones direccion) {
        ...
        if (direccion.getNombre().compareTo(
                inicio.getNodo().getNombre()) < 0) {
            //antes del primer nodo
            nuevo.setApuntSgte(inicio);
            inicio.setApuntAnt(nuevo);
            inicio = nuevo;
        } else if (direccion.getNombre().compareTo(
                fin.getNodo().getNombre()) > 0
                || direccion.getNombre().compareTo(
                fin.getNodo().getNombre()) == 0) {
            ...
            // ubica el anterior
            while (direccion.getNombre().compareTo(
                    auxiliar.getNodo().getNombre()) > 0
                    || direccion.getNombre().compareTo(
                    auxiliar.getNodo().getNombre()) == 0) {
    ...
    public NodoDoble busca(String codigo) {
        // empieza por el primero de la lista
        NodoDoble Auxiliar = inicio;

        // mientras no sea nulo
        while (Auxiliar != null) {
            if (codigo.equals(Auxiliar.getNodo().getNombre())) {
