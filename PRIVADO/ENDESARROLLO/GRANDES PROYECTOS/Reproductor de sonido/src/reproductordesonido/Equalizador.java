...
// Agreammos el evento del popup al boton
    ...
    public void synchronizeEqualizer() 
    {
        ...
        if (btn1.isSelected())
        {
            for (int j = 0; j < eqgains.length; j++)
            {
                ...
                //System.out.println("valores q"+eqgains[j]);
            }
            updateBands(eqgains, minGain, maxGain);
            //System.out.println(eqgains);
        } 
        else
        {
            for (int j = 0; j < eqgains.length; j++) {
    ...
    public void updateBands(int[] gains, int min, int max)
    {
        ...
        //System.out.println("Nueva banda "+bands);
        //setBands(bands);
    }
    ...
    {
    mainpopup.show(eventoMouse.getComponent(), eventoMouse.getX(), eventoMouse.getY());
    }
