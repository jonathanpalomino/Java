...
GUIReproductor abc;
    public Busqueda(ListaDobleConOrden ldco1,GUIReproductor un)
    {
        Busqueda.ldco=ldco1;
        this.abc = un;
        initComponents();
    ...
    /*public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Busqueda(ldco,abc).setVisible(true);
            }
        });
    }*/
    ...
    private File Buscar_Direccion(Object Nombre_item)
    {
        File archivo = null;
    ...
    private void Reproduce(File Direccion_Archivo)
    {
        String ruta = Direccion_Archivo.toString();
        //System.out.println(Direccion_Archivo);
        int filas = abc.getMiTabla().getRowCount();
        for(int h=0;h<filas;h++)
        {
            if(ruta==abc.getMiTabla().getValueAt(h, 1).toString())
            {
                //System.out.println(" igual "+abc.getMiTabla().getValueAt(h, 1));
                abc.Reproducir(h);
                abc.getMiTabla().changeSelection(h, 1, false, false);
            }
        }
    }
